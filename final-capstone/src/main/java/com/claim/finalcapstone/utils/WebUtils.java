			...
			e.printStackTrace();
				...
				//Initialize file
					...
					if(dir.exists()) {
						...
						//Rename file
						File rename = new File(destDir + File.pathSeparator + "user_" +id + ".jpg");
...
public void multipartSave(MultipartFile[] files, long id) throws IllegalStateException, IOException {
	
	try {
		String realPathToUploads = request.getSession().getServletContext().getRealPath(UPLOADED_FOLDER);
		
		File dir = new File(realPathToUploads + File.pathSeparator + id);
		...
		if(files != null && files.length >0) {
			for (MultipartFile file : files) {
				String fileName = file.getOriginalFilename();
				String filePath = realPathToUploads + File.separator + id + File.pathSeparator + fileName;
